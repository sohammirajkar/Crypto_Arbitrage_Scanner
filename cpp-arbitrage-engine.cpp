...
void ArbitrageEngine::start() {
    ...
    if (config_.threading.pin_threads && !config_.threading.cpu_affinity.empty()) {
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
        CPU_SET(config_.threading.cpu_affinity[0], &cpuset);
    ...
    if (config_.threading.pin_threads && config_.threading.cpu_affinity.size() > 1) {
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
        CPU_SET(config_.threading.cpu_affinity[1], &cpuset);
        pthread_setaffinity_np(detection_thread_.native_handle(), sizeof(cpu_set_t), &cpuset);
    }
}
...
void ArbitrageEngine::process_market_data() {
    MarketTick tick;
    const auto wait_time = std::chrono::microseconds(100); // 100μs polling interval
...
void ArbitrageEngine::update_price_graph(const MarketTick& tick) {
    ...
    if (!symbol_pair.has_value()) return;
    
    auto [base, quote] = symbol_pair.value();
    
    // Get indices for base and quote currencies
    size_t base_idx = get_currency_index(base, static_cast<uint8_t>(tick.exchange));
    size_t quote_idx = get_currency_index(quote, static_cast<uint8_t>(tick.exchange));
...
void ArbitrageEngine::detect_arbitrage_opportunities() {
    const auto detection_interval = std::chrono::milliseconds(10); // 100Hz detection
    ...
    while (running_.load()) {
        ...
        auto end_time = std::chrono::high_resolution_clock::now();
        auto detection_latency = std::chrono::duration_cast<std::chrono::microseconds>(
            end_time - start_time);
...
std::vector<ArbitrageOpportunity> ArbitrageEngine::find_negative_cycles() {
    ...
    for (size_t src = 0; src < V; ++src) {
        if (price_graph_[src][src] != 0.0) continue; // Skip inactive currencies
            ...
            for (size_t v = 0; v < V; ++v) {
                if (price_graph_[u][v] != std::numeric_limits<double>::infinity()) {
                    if (dist[u] + price_graph_[u][v] < dist[v]) {
                        ...
                        auto opp = extract_arbitrage_cycle(static_cast<int>(v), parent, dist);
                        if (opp.has_value()) {
...
std::optional<ArbitrageOpportunity> ArbitrageEngine::extract_arbitrage_cycle(
    int cycle_node, const std::vector<int>& parent, const std::vector<double>& dist) {
...
uint32_t ArbitrageEngine::calculate_confidence(const std::vector<int>& path, 
    ...
    auto data_age = std::chrono::duration_cast<std::chrono::milliseconds>(
        now - last_update_time_.load()).count();
    double freshness_confidence = std::max(0.0, 50.0 - data_age / 100.0);
    
    return static_cast<uint32_t>(profit_confidence + path_confidence + freshness_confidence);
}

Volume ArbitrageEngine::estimate_max_volume(const std::vector<int>& path) const {
...
size_t ArbitrageEngine::get_currency_index(const std::string& currency, uint8_t exchange) {
...
void ArbitrageEngine::register_opportunity_callback(OpportunityCallback callback) {
    std::lock_guard<std::mutex> lock(callback_mutex_);
    callbacks_.push_back(std::move(callback));
}

void ArbitrageEngine::on_opportunity_detected(const ArbitrageOpportunity& opportunity) {
    // Rate limiting
    auto now = std::chrono::high_resolution_clock::now();
    static auto last_alert = now;
    static uint32_t alerts_this_second = 0;
    
    if (std::chrono::duration_cast<std::chrono::seconds>(now - last_alert).count() >= 1) {
...
std::vector<ArbitrageOpportunity> ArbitrageEngine::get_recent_opportunities(size_t limit) const {
    std::lock_guard<std::mutex> lock(opportunities_mutex_);
    
    size_t start_idx = detected_opportunities_.size() > limit ? 
                       detected_opportunities_.size() - limit : 0;
    
    return std::vector<ArbitrageOpportunity>(
        detected_opportunities_.begin() + start_idx,
